...
shp = shapefile.Reader("world-administrative-boundaries.zip")
    ...
    for row in reader:
        if row["year"] == "2020":
            co2_data[row["iso_code"]] = (
                float(row["co2_per_capita"]) * 105000 if row["co2_per_capita"] else 0
            )
...
for feature in shp.__geo_interface__["features"]:
    ...
    geometry = force_3d(geometry, co2_emission)
...
document = fastkml.containers.Document(features=continents.values(), styles=styles)
